...
static char *custom_module_blacklist_mayfly[] = {
    /* Not required */
...
custom_blacklist:
	for (i = 0; i < ARRAY_SIZE(custom_module_blacklist); i++)
		if (!strcmp(module_name, custom_module_blacklist[i]))
			return true;
	if (get_xiaomi_hwid_project() == 12 || get_xiaomi_hwid_project() == 8)  // 12 represents mayfly and 8 for UniCorn
		for (i = 0; i < ARRAY_SIZE(custom_module_blacklist_mayfly); i++)
			if (!strcmp(module_name, custom_module_blacklist_mayfly[i]))
				return true;

...
static inline int may_follow_link(struct nameidata *nd, const struct inode *inode)
{
	...
	if (nd->inode && unlikely(nd->inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (inode && unlikely(inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		    ...
		    struct path *path, struct path *root)
{
	...
	putname(name);
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		return -ENOENT;
	}
#endif
	return retval;
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
		error = inode_permission(dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->susfs_app_profile & NON_ROOT_USER_APP_PROFILE)) {
			dput(dentry);
			return ERR_PTR(-ENOENT);
		}
#endif
